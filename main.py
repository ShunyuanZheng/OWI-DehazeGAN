...
class CycleGAN:
    ...
    def __init__(self, pool_size, lambda_a,
                 lambda_b, delta_a, delta_b,output_root_dir, to_restore,
    ...
    def model_test_setup(self):
        """
        This function sets up the model to train.

        self.input_A/self.input_B -> Set of testing images.
        self.fake_A/self.fake_B -> Generated images by corresponding generator
        of input_A and input_B
    ...
    def fake_image_pool(self, num_fakes, fake, fake_pool):
        ...
        pool of images.
        计算每个生成图像的鉴别器损失是不可能的，因为会耗费大量的计算资源。为了加快训练，我们存
        储了之前每个域的所有生成图像，并且每次仅使用一张图像来计算误差。首先，逐个填充图像库使
        其完整，然后随机将某个库中的图像替换为最新的生成图像，并使用这个替换图像来作为该步的训练
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(sess.run(self.global_step), self._max_step):
                ...
                for i in range(0, max_images):
                    ...
                    # Optimizing the D_A network(need fake_A_temp1 because to calculate the error in training D_A)
                    _, summary_str = sess.run(
...
def main(to_train, log_dir, config_filename, checkpoint_dir, skip):
    ...
    delta_a = float(config['_DELTA_A']) if '_DELTA_A' in config else 10.0
    delta_b = float(config['_DELTA_B']) if '_DELTA_B' in config else 10.0
    ...
    cyclegan_model = CycleGAN(pool_size, lambda_a, lambda_b, delta_a, delta_b,log_dir,
